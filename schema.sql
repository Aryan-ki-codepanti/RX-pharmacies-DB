...
CREATE TABLE DOCTOR(
    ...
    DOCTOR_TYPE VARCHAR(25) 
        NOT NULL CHECK(DOCTOR_TYPE IN ('Family', 'Non Family'))
);
...
CREATE TABLE DISPOSED_BY(
    PLANT_ID INTEGER 
        REFERENCES TREATMENT_PLANT(PLANT_ID)
        ON DELETE CASCADE,
...
-- SELLS table
CREATE TABLE SELLS(
    PHARMACY_LOCATION VARCHAR(30)  
        REFERENCES PHARMACY(BRANCH_LOCATION)
        ON DELETE CASCADE ,
